...
* `<optional D key>`: Sending the `D` DTMF tone at the end of the command indicates that a private call should be made on DMR, or to link to the `Echo` repeater on D-STAR.  This is translated to a `#` character and passed to DVSwitch as either a escaped `#` character, or as the `E` echo tone on D-STAR, as appropriate.  Note that `dvswitch.sh` might not support the use of the `#` character to specify a private call on DMR, in which case the talkgroup will be connected as a regular talkgroup.  This may cause some modes like Parrot echo testing to silently fail.
